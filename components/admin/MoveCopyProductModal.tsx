...
export const MoveCopyProductModal: React.FC<MoveCopyProductModalProps> = ({ isOpen, onClose, product }) => {
  ...
  useEffect(() => {
    if (isOpen && product) {
        ...
        setAction('move');
        const availableCategories = settings?.categories?.map(c => c.name).filter(name => name !== product.category) || [];
        setDestinationCategory(availableCategories[0] || '');
    }
  }, [isOpen, product, settings]);

  if (!isOpen || !product) return null;

  const availableMoveCategories = settings?.categories?.map(c => c.name).filter(name => name !== product.category) || [];
  const availableCopyCategories = settings?.categories?.map(c => c.name) || [];
