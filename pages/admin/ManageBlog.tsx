        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start z-50 p-4">
            <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="p-4 border-b">
                    <h2 className="text-2xl font-bold">{post ? 'Edit Post' : 'Add New Post'}</h2>
                </div>
...
const ManageBlog = () => {
    ...
    const handleDelete = (post: UpdatePost) => {
        if (window.confirm(`Are you sure you want to delete this post titled "${post.title}"?`)) {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Manage Blog</h1>
                <Button onClick={() => openModal()} size="md"><Icons.plus className="w-5 h-5 mr-2" /> Add Post</Button>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-x-auto">
                ...
                {updatePosts.length === 0 && !isLoading && <p className="text-center p-8 text-gray-500">No posts found.</p>}
            </div>
